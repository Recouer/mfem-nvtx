...
namespace mfem
{
   ...
   bool BilinearFormIntegrator::has_GPU_support() { return false; }
   ...
   bool ElasticityIntegrator::has_GPU_support()
   {
      return true;
   }
       ...
       DenseTensor &tensor)
   {
      ...
      Array2D<int> IntRule_Sizes(number_of_elements, 2);
      ...
      DenseTensor TransInvJ_values(dim, dim, IntRule_TotalPoints);
      DenseTensor dshape_values(dof, dim, IntRule_TotalPoints);
      if (tensor.TotalSize() == 0)
         tensor.SetSize(dof * dim, dof * dim, number_of_elements);
         ...
         for (size_t j = 0; j < IntRule_Sizes(i, 1); j++)
         {
            ...
            for (size_t k = 0; k < dim; k++)
               for (size_t l = 0; l < dim; l++)
                  TransInvJ_values(k, l, index) = InvJacobian(k, l);

            el.CalcDShape(ip, dshape);
            for (size_t k = 0; k < dim; k++)
               for (size_t l = 0; l < dof; l++)
                  dshape_values(l, k, index) = dshape(l, k);
      ...
      auto GPU_IntRule_Sizes = Reshape(IntRule_Sizes.Read(), number_of_elements * 2);

      auto GPU_TransInvJ = Reshape(TransInvJ_values.Read(), dim, dim, IntRule_TotalPoints);
      auto GPU_dshape = Reshape(dshape_values.Read(), dof, dim, IntRule_TotalPoints);

      auto GPU_elmat = Reshape(tensor.ReadWrite(), dof * dim, dof * dim, number_of_elements);

      auto GPU_w = Reshape(w_values.Read(), IntRule_TotalPoints);
      auto GPU_L = Reshape(L_values.Read(), IntRule_TotalPoints);
      auto GPU_M = Reshape(M_values.Read(), IntRule_TotalPoints);
      // endregion

      // region local values
      DenseMatrix elmat_local(dim * dof), gshape(dof, dim), pelmat(dof);
      Vector divshape(dim * dof);
         ...
         for (size_t p = 0; p < number_of_elements; p++)
         {
            for (size_t i = 0; i < GPU_IntRule_Sizes(p * 2 + 1); i++)
            {
               ...
               for (size_t m = 0; m < dim * dof; m++)
               {
                  for (size_t n = 0; n < dim * dof; n++)
                  {
                     GPU_elmat(m, n, p) += GPU_elmat_local(m, n);
                  }
               }
            }
         }
      ...
      cudaMemcpy(tensor.Data(), GPU_elmat, number_of_elements * dof * dim * dof * dim * sizeof(double), cudaMemcpyDeviceToHost);
       ...
       const FiniteElement &el, ElementTransformation &Trans, DenseMatrix &elmat)
   {
      ...
      // DEBUG("Call to assembleElementMatrix\n");
   ...
   const int numberOfPoints = ir->GetNPoints();

   DenseTensor TransInvJ_values(numberOfPoints, dim, dim), 
               dshape_values(numberOfPoints, dof, dim);
   Vector w_values(numberOfPoints), M_values(numberOfPoints), L_values(numberOfPoints);
   ...
   for (size_t i = 0; i < numberOfPoints; i++)
   {
      ...
      Trans.SetIntPoint(&ip);
      w_values[i] = ip.weight * Trans.Weight();

      auto InvJacobian = Trans.InverseJacobian();
      for (size_t k = 0; k < dim; k++)
         for (size_t l = 0; l < dim; l++)
            TransInvJ_values(i, k, l) = InvJacobian(k, l);
      ...
      for (size_t k = 0; k < dim; k++)
         for (size_t l = 0; l < dof; l++)
            dshape_values(i, l, k) = dshape(l, k);
      

      M_values[i] = mu->Eval(Trans, ip);
      if (lambda)
      {
         L_values[i] = lambda->Eval(Trans, ip);
      }
      else
      {
         L_values[i] = q_lambda * M_values[i];
         M_values[i] = q_mu * M_values[i];
      }
   }
   

      auto GPU_TransInvJ = Reshape(TransInvJ_values.Read(), numberOfPoints, dim, dim);
      auto GPU_dshape = Reshape(dshape_values.Read(), numberOfPoints, dof, dim);

      auto GPU_elmat = Reshape(elmat.ReadWrite(), dof*dim, dof*dim);
      auto GPU_pelmat = Reshape(pelmat.ReadWrite(), dof, dof);
      auto GPU_divshape = Reshape(divshape.ReadWrite(), dim*dof);
      auto GPU_gshape = Reshape(gshape.ReadWrite(), dof, dim);

      auto GPU_w = Reshape(w_values.Read(), numberOfPoints);
      auto GPU_L = Reshape(M_values.Read(), numberOfPoints);
      auto GPU_M = Reshape(L_values.Read(), numberOfPoints);
